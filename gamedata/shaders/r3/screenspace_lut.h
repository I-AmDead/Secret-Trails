...
float3 ssfx_lut_pp(float3 base_col)
{
    ...
    // Sample LUTs
    float3 lut_col = lerp(s_lut_atlas.Sample(smp_rtlinear, uvs.xy + grp_offset).rgb, s_lut_atlas.Sample(smp_rtlinear, uvs.zw + grp_offset).rgb, lut_frac);
    ...
    float3 SecondLUT = lerp(s_lut_atlas.Sample(smp_rtlinear, uvs.xy + grp_offset).rgb, s_lut_atlas.Sample(smp_rtlinear, uvs.zw + grp_offset).rgb, lut_frac);
