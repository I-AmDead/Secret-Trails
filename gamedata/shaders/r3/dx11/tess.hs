...
#ifndef TESS_HM
    bool doDiscard = all(N[0].z > 0.1) && all(N[1].z > 0.1) && all(N[2].z > 0.1) && all(Output.patch.f3N110.z > 0.1) && all(Output.patch.f3N011.z > 0.1) &&
        all(Output.patch.f3N101.z > 0.1) && all(P[0].z > 5) && all(P[1].z > 5) && all(P[2].z > 5);
