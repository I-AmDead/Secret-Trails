...
#ifdef SSFX_PUDDLES
#include "common\screenspace\settings_screenspace_PUDDLES.h"
#endif
...
// Puddles slope mask and height ----------------------------------
    ...
    float wetness_f = 1.0f;
#else
    float wetness_f = saturate(rain_params.y) * ssfx_terrain_pom.w;
#endif

    const bool need_render_puddles = wetness_f > 0.0f && I.position.z < TERRAIN_PUDDLES_RANGE;
    if (need_render_puddles)
    {
        float height_str = saturate(wetness_f * TERRAIN_POM_PLANE - PixelHeight);
        puddles = saturate(height_str * 4.0f);
    ...
    if (rain_params.x > 0.0f && I.position.z < TERRAIN_PUDDLES_RIPPLES_RANGE)
        {
            ...
            ripplesNormal = normalize(float3(WN0.xy + WN1.xy, WN0.z * WN1.z)) * 2.0f - 1.0f; //((WN0 + WN1) * 0.5f) * 2.0f - 1.0f;
    ...
    f_deffer O = pack_gbuffer(float4(Ne, C.w), float4(offset_pos, 0.95f), float4(C.rgb, G));
