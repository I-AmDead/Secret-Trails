...
{
    // ??????????? ???????? ??? ???????????? ????????? ?????? ?????
    I.tc0.x = resize(I.tc0.x, screen_res.y / screen_res.x, 0.5);
    I.tc0.x = resize(I.tc0.x, screen_res.x / screen_res.y, 0.5);
    // I.tc0.x = resize(I.tc0.x, 1.25, 0.0); // ??? ??????????????? ??? ??? ui
