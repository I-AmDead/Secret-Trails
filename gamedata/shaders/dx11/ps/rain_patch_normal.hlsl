...
float4 main(float2 tc : TEXCOORD0, float2 tcJ : TEXCOORD1, float4 Color : COLOR, float4 pos2d : SV_Position) : SV_Target
{
    ...
    float3 WorldN = mul(m_sunmask, normal);

#ifdef INDOOR_LEAKS_FIX
    float Cover = shadow_hw(PS) * saturate(hemi * 10.0f);
#else
    ...
    // Distance
    float p_len = length(view_space);
    bool is_flora = abs(material - 0.15) < 0.04f;
    bool is_terrain = abs(material - 0.95) < 0.04f;
