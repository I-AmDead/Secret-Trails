...
float4 main(p_fluidsim input) : SV_Target
{
    ...
    float3 npos = input.cell0 - timestep * forward * Texture_velocity0.SampleLevel(samPointClamp, input.texcoords, 0).xyz;
    ...
    r = Texture_phi_n.SampleLevel(samLinear, nposTC, 0) +
        0.5 * (Texture_phi_n.SampleLevel(samPointClamp, input.texcoords, 0) - Texture_phi_n_hat.SampleLevel(samPointClamp, input.texcoords, 0));
