...
// Settings
#include "common\screenspace\settings_screenspace_LUT.h"
...
float3 img_lut(float3 base_col)
{
    ...
    // Group offset
    float2 grp_offset = float2(0.0, lut_params.y / LUT_GROUPS);

    // Sample LUTs
    float3 lut_col = lerp(s_lut_atlas.Sample(smp_linear, uvs.xy + grp_offset).rgb, s_lut_atlas.Sample(smp_linear, uvs.zw + grp_offset).rgb, lut_frac);

#ifdef G_ADVANCE_TRANSITION

    // Group2 offset
    grp_offset = float2(0.0, lut_params.z / LUT_GROUPS);

    // Sample transition LUTs
    float3 SecondLUT = lerp(s_lut_atlas.Sample(smp_linear, uvs.xy + grp_offset).rgb, s_lut_atlas.Sample(smp_linear, uvs.zw + grp_offset).rgb, lut_frac);

    lut_col = lerp(lut_col, SecondLUT, lut_params.w);
#endif

    return lerp(base_col.rgb, lut_col.rgb, lut_params.x);
}